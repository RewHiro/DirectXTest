...
HRESULT Dx12Wrapper::CreateSceneView()
{
	...
	_mappedSceneData->invProj = XMMatrixInverse(&det, _mappedSceneData->proj);
...
HRESULT Dx12Wrapper::CreatePeraResource()
{
	...
	for (auto& resource : _peraResources)
	{
		result = _dev->CreateCommittedResource
		(
			&heapProp
	...
	materialDescHeapDesc.NumDescriptors = 7;
	...
	srvDesc.Format = _peraResources[0]->GetDesc().Format;
	...
	range[3].BaseShaderRegister = 6; // 1
	range[3].NumDescriptors = 1;

	range[4].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; // t
	range[4].BaseShaderRegister = 7; // 1
	...
	(
		...
		nullptr, // defineはなし
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルードはデフォルト
		"BlurPS", "ps_5_0", // 関数はmain、対象シェーダーはps_5_0
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用および最適化はなし
		0,
		ps.ReleaseAndGetAddressOf(), errorBlob.ReleaseAndGetAddressOf() // エラー時はerrorBlobにメッセージが入る
	);
	...
	(
		...
		nullptr, // defineはなし
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルードはデフォルト
		"SSaoPs", "ps_5_0", // 関数はmain、対象シェーダーはvs_5_0
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用および最適化はなし
		0,
		ps.ReleaseAndGetAddressOf(), errorBlob.ReleaseAndGetAddressOf()// エラー時はerrorBlobにメッセージが入る
	);
...
bool Dx12Wrapper::CreateAmbientOcclusionPipeline()
{
	D3D12_INPUT_ELEMENT_DESC layout[2] =
	{
		{
	...
	_parallelLightVec(1, -1, 1)
{
	...
	//if (!CreateAmbientOcclusionPipeline())
	//{
	//	assert(0);
	//	return;
	//}
...
{
	for (auto& resouce : _peraResources)
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(resouce.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
		_cmdList->ResourceBarrier(1, &barrier);
	}

	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_bloomBuffer[0].Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
		_cmdList->ResourceBarrier(1, &barrier);
	}
	...
	{
		handle.InitOffsetted(rtvH, offset);
		offset += incSize;
	}

	_cmdList->OMSetRenderTargets(3, rtvs, false, &dsvH);
...

	{
		...
		_cmdList->ClearRenderTargetView(rtvs[2], clearColor, 0, nullptr);
	}
...
void Dx12Wrapper::PostDrawToPera1()
{
	for (auto& resouce : _peraResources)
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(resouce.Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
		_cmdList->ResourceBarrier(1, &barrier);
	}
}
...
void Dx12Wrapper::DrawShrinkTextureForBlur()
{
	...
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_bloomBuffer[0].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
		_cmdList->ResourceBarrier(1, &barrier);
	}

	// 縮小バッファーはレンダーターゲットに
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_bloomBuffer[1].Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
		_cmdList->ResourceBarrier(1, &barrier);
	}

	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_dofBuffer.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
		_cmdList->ResourceBarrier(1, &barrier);
	}

	auto rtvHandle = _peraRTVHeap->GetCPUDescriptorHandleForHeapStart();
	auto srvHandle = _peraRegisterHeap->GetGPUDescriptorHandleForHeapStart();

	CD3DX12_CPU_DESCRIPTOR_HANDLE rtvs[2];

	auto incSize = _dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

	rtvs[0].InitOffsetted(rtvHandle, incSize * 3);
	rtvs[1].InitOffsetted(rtvHandle, incSize * 4);

	//float clearColor[] = { 0.5f,0.5f,0.5f,1.0f };
	//_cmdList->ClearRenderTargetView(rtvHandle, clearColor, 0, nullptr);

	// レンダーターゲットセット
	_cmdList->OMSetRenderTargets(2, rtvs, false, nullptr);
	...
	//srvHandle.ptr += _dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) * 6;
	//_cmdList->SetDescriptorHeaps(1, _peraRegisterHeap.GetAddressOf());
	...
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_bloomBuffer[1].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
		_cmdList->ResourceBarrier(1, &barrier);
	}

	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_dofBuffer.Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
		_cmdList->ResourceBarrier(1, &barrier);
	}

}

void Dx12Wrapper::DrawAmbinetOcclusion()
{
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_aoBuffer.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
		_cmdList->ResourceBarrier(1, &barrier);
	}
	...
	{
		auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(_aoBuffer.Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
		_cmdList->ResourceBarrier(1, &barrier);
	}
}
